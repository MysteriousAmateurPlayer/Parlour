...
首页第一屏是**太阳**（姓名与简介就放在太阳上，古典的锥形 + 火焰光芒），下缘露出**地球的上沿**；往下拉，这颗地球完整展开成**可旋转的地球仪**，球面上的拼图式片区就是各板块入口（空位留给以后新增的板块）。整体保持纸色 + 棕色线稿的星图质感，不做夜空反色；再往下是 关于我 → 板块卡片 → 最近的记录。里版入口是星野里那颗**长得不太一样的星星**，点击弹出问答。
